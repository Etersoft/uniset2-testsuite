...
class XListDialog():
    ...
    def __init__(self,datdir):
        ...
        self.model = gtk.ListStore(gobject.TYPE_STRING,gobject.TYPE_STRING,gobject.TYPE_STRING,gobject.TYPE_STRING,object,object)
...
#               return (model, pathlist)               
    ...
    def on_confbox_changed(self,cbox):
        self.allConf = True
        mod = self.cbox.get_model()
        if self.cbox.get_active_iter()!=None and mod.get_value(self.cbox.get_active_iter(),0) != mod.get_value(mod.get_iter_first(),0):
           self.allConf = False
