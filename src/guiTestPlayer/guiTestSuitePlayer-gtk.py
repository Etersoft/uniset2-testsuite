...
#              self.builder.get_object("mi_addCheck").set_sensitive(True)
    ...
    def on_mi_add_check_activate(self,mitem):
        ...
        it1 = c_iter
        if t == tt.Test:
           it1 = self.model.append(c_iter,[None,'','',"New check",bg_DEFAULT,new_xmlnode,tt.Check,0,0,False,True,0,xml,"",None,False,True,None,False,None])
        else:
           it1 = self.model.insert_after(None,c_iter,[None,'','',"New check",bg_DEFAULT,new_xmlnode,tt.Check,0,0,False,True,0,xml,"",None,False,True,None,False,None])
    ...
    def on_mi_add_test_activate(self,mitem):
        ...
        it1 = self.model.insert_after(None,c_iter,[None,'','',"New test",bg_DEFAULT,new_xmlnode,tt.Test,0,0,False,True,0,xml,"",None,False,True,None,False,None])
        it2 = model.convert_child_iter_to_iter(it1)
        self.on_edit(model,it2,True)
    ...
    def build_test_scenario(self,xml,num=1,iter=None, prefix_num=""):
        ...
        while node != None:
              ...
              it = self.model.append(iter,[None,'','',node.prop("name"),bg_DEFAULT,node,tt.Test,0,0,False,True,num,xml,s_num,None,False,True,None,False,None])
    ...
    def build_test(self,iter,xml,node,t_num,prefix_num):
        ...
        while node != None:
            ...
            if node.name == "check":
               ...
               if test == "LINK":
                  it = self.model.append(iter,[None,'','',txt,bg_DEFAULT,node,tt.Link,0,0,False,True,num,xml,s_num,None,False,True,None,False,None])
                  self.build_link(it,node,xml,num,s_num)
               elif test == "OUTLINK":
                  it = self.model.append(iter,[None,'','',txt,bg_DEFAULT,node,tt.Outlink,0,0,False,True,num,xml,s_num,None,False,True,None,False,None])
                  self.build_outlink(it,node,num,s_num)
               else:
                  it = self.model.append(iter,[img,'','',txt,bg_DEFAULT,node,tt.Check,0,0,False,True,num,xml,s_num,None,False,True,None,False,None])
                  self.set_replace_list(it,node)
            elif node.name == "action":
               txt = self.tsi.get_action_info(node)
               it = self.model.append(iter,[img,'','',txt,bg_DEFAULT,node,tt.Action,0,0,False,True,num,xml,s_num,None,False,True,None,False,None])
            else:
    ...
    def update_info(self,iter,old_name):
        ...
        if t == tt.Check:
           ...
           txt = self.tsi.get_check_info(xmlnode)
           it1 = self.model.insert_after(None,iter,[None,'','',txt,bg_DEFAULT,xmlnode,tt.Link,0,0,False,True,num,xml,s_num,None,False,True,None,False,None])
           ...
           s_num = self.model.get_value(iter,fid.t_num)
           txt = self.tsi.get_check_info(xmlnode)
           it1 = self.model.insert_after(None,iter,[None,'','',txt,bg_DEFAULT,xmlnode,tt.Outlink,0,0,False,True,num,xml,s_num,None,False,True,None,False,None])
           self.tv.set_cursor( self.model.get_path(it1) )
           self.model.remove(iter)
    ...
    def on_play(self, begin_iter):
        ...
        if self.start_pmonitor(self.play_iter) == False:
           ...
           print "*** FAILED RUN LIST for (%s) '%s'"%(self.model.get_value(self.play_iter,fid.xml).getFileName(),self.model.get_value(self.play_iter,fid.tname))
           self.play_scenario = False
           return
...
#        print "**************** pmonitor_terminate: %s"%err
    ...
    def on_finish_test(self,iter):
        ...
        self.to_log(res[0],"Завершили тест %s. %s"%(self.model.get_value(iter,fid.t_num),(self.model.get_value(iter,fid.tname))))
...
#        self.tv.columns_autosize()
    ...
    def on_play_item(self,iter):
        ...
        try:
            ...
            self.to_log(res,"%s. '%s'"%(self.model.get_value(iter,fid.t_num),(self.model.get_value(iter,fid.tname))))
...
#        self.log_model.prepend([dt.strftime("%Y-%m-%d %H:%M:%S"),res,txt])
    ...
    def save_report_to_file(self, filename):
        ...
        while it is not None:
           s = "%19s|%10s|%s\n"%(self.log_model.get_value(it,lid.dt),self.log_model.get_value(it,lid.res),self.log_model.get_value(it,lid.txt))
           out.write(s)
