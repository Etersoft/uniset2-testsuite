...
class TestSuiteInterface():
    ...
    def add_uniset_config(self, xmlfile, alias, already_ignore=True):
        ...
        if alias in self.ui_list:
           if already_ignore == False:
              self.log(t_FAILED,"(add_uniset_config): %s already added..(Ignore add '%s@%s')"%(alias,alias,xmlfile), True)
              return None
    ...
    def add_modbus_config(self, mbslave, alias, already_ignore=True):
        if alias in self.ui_list:
           if already_ignore == False:
              self.log(t_FAILED,"(add_modbus_config): %s already added..(Ignore add '%s@%s')"%(alias,alias,xmlfile), True)
              return None
           return self.ui_list[alias]
    ...
    def set_default_config(self, xmlfile, already_ignore=True):
        if xmlfile in self.ui_list:
           if already_ignore == False:
              self.log(t_FAILED,"(set_default_config): %s already added..(Ignore add '%s@%s')"%(alias,alias,xmlfile), True)
              return False
